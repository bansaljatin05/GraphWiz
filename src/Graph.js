...
class Graph extends Component {
  ...
  addEdge2() {
    if (this.state.CVS.length > 1) {
      if (this.state.CVS[0].my_class === this.state.CVS[1].my_class) {
        ...
        newedgelist[parseInt(this.state.CVS[0].my_class) - 1] = newedgelist[
          parseInt(this.state.CVS[0].my_class) - 1
        ].concat([parseInt(this.state.CVS[1].my_class) - 1]);
        newedgelist[parseInt(this.state.CVS[1].my_class) - 1] = newedgelist[
          parseInt(this.state.CVS[1].my_class) - 1
        ].concat([parseInt(this.state.CVS[0].my_class) - 1]);
  ...
  makeGraph(graph) {
    ...
    for (i = 0; i < array[1]; i++) {
      ...
      edgelist[parseInt(array[j - 2]) - 1] = edgelist[
        parseInt(array[j - 2]) - 1
      ].concat([parseInt(array[j - 1]) - 1]);
      edgelist[parseInt(array[j - 1]) - 1] = edgelist[
        parseInt(array[j - 1]) - 1
      ].concat([parseInt(array[j - 2]) - 1]);
    }
    console.log("DONE");
    this.setState(
      {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      (this.props.v1 > 0) &
      (this.props.v1 <= this.state.count) &
      (this.props.v2 > 0) &
      (this.props.v2 <= this.state.count) &
      (nextProps.vax3 === 1) &
      !(this.props.v1 === this.props.v2)
    ) {
      const newedgelist = this.state.edgelist;
      newedgelist[parseInt(this.props.v1) - 1] = newedgelist[
        parseInt(this.props.v1) - 1
      ].concat([parseInt(this.props.v2) - 1]);
      newedgelist[parseInt(this.props.v2) - 1] = newedgelist[
        parseInt(this.props.v2) - 1
      ].concat([parseInt(this.props.v1) - 1]);
      console.log(newedgelist);
      this.setState(
        {
          edges: this.state.edges.concat({
  ...
  BFS = (event) => {
    ...
    if (l === "transparent" || l === "App-edge") {
      ...
      for (x of this.state.edges) {
        ...
        for (y of newEdges) {
          if (
            String(x.class1.my_class) === String(y.class1.my_class) &&
            String(x.class2.my_class) === String(y.class2.my_class)
          ) {
            flag = 1;
            x.visited = y.visited;
          }
          if (
            String(x.class1.my_class) === String(y.class2.my_class) &&
            String(x.class2.my_class) === String(y.class1.my_class)
          ) {
            flag = 1;
            x.visited = y.visited;
          }
